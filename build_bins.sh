...
# Snag elasticsearch
elasticsearch_dpkg="elasticsearch-0.90.3.deb"
if ! [[ -f $elasticsearch_dpkg ]]; then
  $CURL -O -L https://download.elasticsearch.org/elasticsearch/elasticsearch/$elasticsearch_dpkg
fi
FILES="$elasticsearch_dpkg $FILES"
...
if ! [[ -f python-carbon_0.9.10_all.deb && \
  ...
  fpm --python-install-bin /opt/graphite/bin -s python -t deb -v 0.9.10 carbon
  fpm --python-install-bin /opt/graphite/bin  -s python -t deb -v 0.9.10 whisper
  fpm --python-install-lib /opt/graphite/webapp -s python -t deb -v 0.9.10 graphite-web
fi
